...
import { isApproved } from "./access.js";
...
// user — or sends a 401/403 and returns null so the endpoint can just
// `return`. Now async: verifying the signature alone used to be enough to
// let someone in, but that only proves "this is a real Telegram user," not
// "this user is allowed in" — that's the isApproved check below.
export async function requireTelegramUser(req, res) {
  ...
  if (!(await isApproved(user.id))) {
    res.status(403).json({
      error: "not_approved",
      message: "You don't have access to this bot yet — message it on Telegram to request access.",
    });
    return null;
  }
  return user;
